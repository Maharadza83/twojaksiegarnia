...
namespace ksiegarnia.Repositories.Implementation
{
    public class MovieService : IMovieService
    {
        ...
        public MovieListVm List(string term = "", bool paging = false, int currentPage = 0)
        {
            ...
            {
                term = term.ToLower();
                // Wyszukaj filmy po tytule lub gatunku
                list = list.Where(a => a.Title.ToLower().Contains(term) || a.Cast.ToLower().Contains(term)).ToList();
            }

            if (paging)
            {
                // Tutaj zastosujemy stronicowanie
                int pageSize = 5;
